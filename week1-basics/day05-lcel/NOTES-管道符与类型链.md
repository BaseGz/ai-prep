...
>
> （这 3 行已封装成 `lcel_demo.py` 里的 `check_console_buffer()`，**默认不调用**，要测时手动跑一次即可。）

---

## 18. D5 验收自测（七道题）

> 来源：`docs/daily/D05-2-LCEL实操与验证.html` 第 5 节。
> **先不看答案、不看代码自答一遍，再对下面这张表。**
> 判过标准：**答对 ≥5 题，且第 7 题必须对。**

**题目**

1. `prompt | model` 里，`|` 到底做了什么？
2. 为什么 `chain.invoke()` 要传 `{"question": "..."}` 而不是 `"..."`？是链的**哪一端**决定了这件事？
3. 去掉 `StrOutputParser()` 后 `print` 出来是什么类型？为什么是这个类型？
4. `stream` 和 `batch` 各自解决什么问题？**能同时用吗？**
5. 为什么 `batch` 通常比 `for + invoke` 快？瓶颈在哪？
6. `asyncio.run(...)` 为什么必须有？
7. **一句话**说清管道符做了什么（这题是唯一必须答对的）

**答案要点**

| # | 答案要点（关键词命中即算对） | 详见 |
|---|---|---|
| 1 | 管道符是 `Runnable` 重载的 `__or__`，拼成 `RunnableSequence`；语义是「前者的输出成为后者的输入」。**说得出「重载」或「RunnableSequence」任一个就够** | §1、§4 |
| 2 | **链的入口**是 `ChatPromptTemplate`，它要的是变量字典（对比：`model.invoke()` 收消息列表 —— 看入口是谁）。**满分要加半句**：单变量模板传字符串其实也会被接受（自动塞进唯一变量），但那是**特例不是规则** → 规范做法永远显式传 dict | §5、§15 |
| 3 | `AIMessage`。模型吐的就是消息对象，**不接 parser 就没有那一步转换** —— 这不是报错，是类型没转 | §12、§13 |
| 4 | `stream` = 逐块返回（打字机效果）；`batch` = 一次输入多个（内部并发）。**能同时用** → `abatch()` / `astream()` | §16 |
| 5 | `batch` 内部**并发**发请求，`for` 是**串行**等。**瓶颈在网络往返（IO），不在 CPU** —— 所以并发的收益才明显 | — |
| 6 | 顶层代码不能 `await`，得有个**事件循环**来跑协程；`asyncio.run` 负责建循环、跑、收尾 | §16 第 10 条 |
| 7 | **「上一个的输出，接给下一个当输入」** —— 只要能说出这句就算过 | §4 |
